...
class PostController extends Controller
{
 ...
 public function DeleteBlogCat($id){
 	DB::table('post_category')->where('id',$id)->delete();
      ...
      $notification=array(
            'massage'=>'Post Category Delete successfully',
            'alert-type'=>'success'
      );
           return Redirect()->back()->with($notification);
   ...
   public function EditBlogCat($id){
    ...
    public function UpdateBlogCat(Request $request,$id){
	    ...
	    $notification=array(
            'massage'=>'Post Category Update successfully',
            'alert-type'=>'success'
      );
  ...
  if ($post_image) {
     ...
     Image::make($post_image)->resize(400,200)->save('image/product/'.$post_image_name);
     $data['post_image'] = 'image/product/'.$post_image_name;
    ...
    DB::table('posts')->insert($data);
        $notification=array(
            'massage'=>'Post Inserted Without Image successfully',
            'alert-type'=>'success'
      );
           return Redirect()->back()->with($notification);
  ...
  if ($post_image) {
   // unlink($oldimage);
    ...
    $data['post_image']= $oldimage;
     DB::table('posts')->where('id',$id)->update($data);
       $notification=array(
            'massage'=>'Post Update without image successfully',
            'alert-type'=>'success'
      );
            return Redirect()->route('all.blogpost')->with($notification);
